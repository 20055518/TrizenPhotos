...
export const EventDetail: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* Header Banner */}
      <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl mb-8 backdrop-blur-md">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-3 mb-2">
              ...
              {event.gallery?.is_published ? (
                <span className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 shadow-sm">
                  <ShieldCheck className="w-3.5 h-3.5" />
                  Gallery Live
                </span>
              ) : (
                <span className="text-xs font-semibold px-3 py-1 rounded-full bg-slate-800 text-slate-400 border border-slate-700/60">
                  Unpublished Draft
                </span>
              )}
            </div>
            ...
            {event.description && (
              <p className="text-xs text-slate-400 mt-3 max-w-2xl">{event.description}</p>
            )}
          </div>

          {/* Operational Metrics Callout */}
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 bg-slate-950/60 p-4 rounded-2xl border border-slate-800/80">
            <div className="px-4 py-2 text-center border-b sm:border-b-0 sm:border-r border-slate-800">
              <div className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">
                Total Uploaded
              </div>
              <div className="text-2xl font-black text-white mt-0.5">{event.total_photos}</div>
            </div>
            ...
            <div className="flex items-center sm:pl-2">
              <button
                ...
                className="w-full sm:w-auto px-5 py-3 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-semibold text-xs rounded-xl shadow-lg shadow-indigo-600/30 transition-all flex items-center justify-center space-x-2 cursor-pointer"
              >
                <Globe className="w-4 h-4" />
                <span>{event.gallery?.is_published ? 'Manage Gallery & PIN' : 'Publish Gallery'}</span>
              </button>
            </div>
          </div>
        </div>
        ...
        {event.gallery?.is_published && (
          <div className="mt-6 pt-6 border-t border-slate-800/80 flex flex-wrap items-center justify-between gap-4 bg-indigo-950/20 -mx-6 -mb-6 p-6 rounded-b-3xl border-b border-indigo-500/20">
            <div className="flex items-center space-x-3">
              <div className="w-8 h-8 rounded-lg bg-indigo-500/10 border border-indigo-500/30 flex items-center justify-center text-indigo-400">
                <Lock className="w-4 h-4" />
              </div>
              <div>
                <div className="text-xs font-semibold text-white">Customer Gallery is Active</div>
            ...
            <div className="flex items-center space-x-2">
              ...
              >
                ...
                <span>{copiedLink ? 'Copied URL' : 'Copy Gallery Link'}</span>
              </button>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center space-x-2.5">
          ...
          <button
            ...
            className="px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold rounded-lg flex items-center space-x-1.5 shadow transition-all cursor-pointer"
          >
      ...
      {filteredPhotos.length === 0 ? (
        <div className="py-20 text-center border border-dashed border-slate-800 rounded-3xl bg-slate-900/30">
          <p className="text-slate-400 text-sm mb-3">No photos match the current filter.</p>
          <button
            onClick={() => fileInputRef.current?.click()}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-4">
          {filteredPhotos.map((photo, index) => {
            ...
            return (
              <div
                key={photo.id}
                className={`group relative rounded-2xl overflow-hidden border transition-all duration-200 bg-slate-900 ${
                  isSelected
              ...
              >
                ...
                <button
                  ...
                  className={`absolute top-2.5 left-2.5 p-1.5 rounded-lg border transition-all cursor-pointer shadow-md ${
                    isSelected
                      ? 'bg-emerald-500 border-emerald-400 text-white'
                      : 'bg-black/60 border-white/20 text-white/40 hover:text-white'
                  }`}
                  title={isSelected ? 'Remove from Published Gallery' : 'Select for Published Gallery'}
                >
                ...
                <div className="absolute top-2.5 right-2.5 flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    onClick={() => setLightboxIndex(index)}
                    className="p-1.5 bg-black/60 hover:bg-black/90 text-white rounded-lg border border-white/20 shadow-md backdrop-blur-sm cursor-pointer"
                  ...
                  <button
                    ...
                    className="p-1.5 bg-rose-950/80 hover:bg-rose-600 text-rose-300 hover:text-white rounded-lg border border-rose-500/30 shadow-md backdrop-blur-sm cursor-pointer"
                    title="Delete Photo"
                  >
                ...
                {/* Photo Card Footer */}
                <div className="p-2.5 bg-slate-900 border-t border-slate-800/80 flex items-center justify-between text-[11px]">
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center space-x-3 mb-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-white">Publish Customer Gallery</h2>
                <p className="text-xs text-slate-400">Generate secure PIN protected guest access</p>
              </div>
            </div>
            ...
            {publishSuccess ? (
              <div className="space-y-4 py-2">
                <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl text-center">
                  ...
                  <h4 className="text-sm font-bold text-white">Gallery Published Successfully!</h4>
                  <p className="text-xs text-slate-300 mt-1">
                    {event.selected_photos} photos are now securely accessible to clients with this PIN.
                  </p>
                </div>
                ...
                <div className="bg-slate-800/80 p-3.5 rounded-xl border border-slate-700/80 space-y-2">
                  ...
                  <div>
                    ...
                    <div className="flex items-center justify-between mt-1 text-lg font-bold tracking-widest text-amber-400 font-mono bg-slate-900 p-2 rounded-lg border border-slate-800">
                      <span>{pin}</span>
                      <button
                        onClick={() => copyToClipboard(pin, 'pin')}
                        className="text-xs text-slate-400 hover:text-white flex items-center gap-1 font-sans font-normal tracking-normal cursor-pointer"
                      >
              ...
              <form onSubmit={handlePublishGallery} className="space-y-4">
                <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl text-xs text-amber-300">
                  Currently, <strong>{event.selected_photos}</strong> out of {event.total_photos} photos will be visible to clients.
                </div>

                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1">
                    Access PIN * (Client will enter this to view)
                  </label>
                  <input
                    type="text"
                    required
                  ...
                  <span className="text-[11px] text-slate-500 mt-1 block">
                    Default operational PIN: 482917
                  </span>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center">
                    <span className="px-3 py-2 bg-slate-800/80 border border-r-0 border-slate-700 rounded-l-xl text-xs text-slate-400 font-mono">
                      /gallery/
                    </span>
                    <input
                      type="text"
                ...
                <div className="flex items-center justify-end space-x-3 pt-3 border-t border-slate-800">
                  <button
                    type="button"
                    onClick={() => setIsPublishModalOpen(false)}
                    className="px-4 py-2 text-slate-400 hover:text-white text-xs font-semibold rounded-lg cursor-pointer"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-xs font-semibold rounded-xl shadow transition-colors cursor-pointer"
                  >
                    {isPublishing ? 'Publishing...' : 'Save & Publish Gallery'}
                  </button>
                </div>
              </form>
            )}
