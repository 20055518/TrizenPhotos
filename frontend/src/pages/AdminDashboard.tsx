    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-800">
        ...
        <button
          ...
          className="inline-flex items-center space-x-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-semibold rounded-xl shadow-lg shadow-indigo-600/30 transition-all hover:scale-[1.02] cursor-pointer"
        >
      ...
      {isLoading ? (
        ...
        <div className="py-16 text-center border border-dashed border-slate-800 rounded-2xl my-8 p-8 bg-slate-900/30">
            ...
            return (
              ...
              >
                <div className="p-6">
                  ...
                  {/* Photo Metrics */}
                  <div className="grid grid-cols-2 gap-2 bg-slate-800/50 p-3 rounded-xl border border-slate-800/80 mb-4">
                    <div>
                  ...
                  {/* Team Members */}
                  <div className="mb-2">
                    <div className="text-[11px] uppercase tracking-wider text-slate-500 font-semibold mb-1.5 flex items-center gap-1">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {event.assigned_team_members && event.assigned_team_members.length > 0 ? (
                        ...
                        <span className="text-xs text-slate-500 italic">No photographers assigned</span>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 bg-slate-900 border-t border-slate-800 flex items-center justify-between gap-2">
                  <div className="flex items-center space-x-2">
                    <Link
                      to={`/events/${event.id}`}
                      className="inline-flex items-center space-x-1.5 px-3 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold rounded-lg shadow transition-colors"
                    >
                    ...
                    {isPublished && event.gallery?.slug && (
                      <button
                        ...
                        className="p-2 text-slate-300 hover:text-white bg-slate-800 hover:bg-slate-700 rounded-lg border border-slate-700 transition-colors flex items-center gap-1 text-xs cursor-pointer"
                      >
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-lg w-full p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-150">
            <h2 className="text-xl font-bold text-white mb-1">Create New Photography Event</h2>
            ...
            <form onSubmit={handleCreateEvent} className="space-y-4">
              ...
              <div>
                ...
                <div className="max-h-36 overflow-y-auto space-y-1.5 p-2 bg-slate-800/50 rounded-xl border border-slate-700/60">
                  {teamMembers.length === 0 ? (
                    ...
                    teamMembers.map((tm) => (
                      ...
                      >
                        ...
                        <span className="text-slate-500 text-[11px]">({tm.email})</span>
                      </label>
                    ))
                  )}
                </div>
